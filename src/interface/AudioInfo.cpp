...
void AudioInfo::setFile(WAVFile *file) {
    ...
    labels << "Subchunk 1 ID" << "Subchunk 1 size" << "Audio format"
           << "Number of channels" << "Sample rate" << "Byte rate"
    ...
    QVector<QVector<std::string>> items = QVector < QVector
            < std::string >> (rows, items_columns);
    // Items
    items[0][0] = "Chunk ID";
    ...
    for (int row = 0; row < rows; row++) {
        for (int column = 0; column < columns; column++) {
            QTableWidgetItem *item = new QTableWidgetItem(
                        QString::fromStdString(items[row][column]));
