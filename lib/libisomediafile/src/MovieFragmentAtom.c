...
static MP4Err mdatMoved( struct MP4MovieAtom* s, u64 mdatBase, u64 mdatEnd, s32 mdatOffset )
{
	...
	for ( i = 0; i < trackCount; i++ )
	{
		...
		err = MP4GetListEntry( self->atomList, i, (char**) &traf ); if (err) goto bail;
		if ( traf == NULL )
			BAILWITHERROR( MP4InvalidMediaErr );
		err = traf->mdatMoved( (struct MP4MediaInformationAtom *) traf, mdatBase, mdatEnd, mdatOffset ); if (err) goto bail;
	}
bail:
	TEST_RETURN( err );
	return err;
...
static MP4Err mergeFragments( struct MP4MovieFragmentAtom* self, MP4MovieAtomPtr moov )
{
	...
	for ( i = 0; i < trackCount; i++ )
	{
		...
		err = moov->getTrackExtendsAtom( moov, tfhd->trackID, (MP4AtomPtr*) &trex ); if (err) goto bail;
		...
		if ((tfhd_flags & tfhd_base_data_offset_present)==0)
			tfhd->base_data_offset = base_offset;
			else base_offset = tfhd->base_data_offset;
			 
		err = traf->calculateDataSize( traf, &traf_data_size ); if (err) goto bail;
		base_offset += traf_data_size;
		
		err = moov->getTrackMedia( moov, tfhd->trackID, (MP4AtomPtr*) &mdia ); if (err) goto bail;
		...
		{
			/* if there is not already an edit list, and there is media, insert an edit covering the current track duration */
			...
			err = MP4InsertMediaIntoTrack( trak, -1, -1, tfhd->default_sample_duration, 1 ); if (err) goto bail;
		}
		else
		{
			...
			if (((MP4TrackAtomPtr) trak)->trackEditAtom) {
				err = MP4InsertMediaIntoTrack( trak, -1, (s32) initialMediaDuration, mediaDuration-initialMediaDuration, 1 ); 
				if (err) goto bail;
			}
		}
...
static MP4Err addAtom( MP4MovieFragmentAtomPtr self, MP4AtomPtr atom )
{
	...
	if ( self == 0 )
		BAILWITHERROR( MP4BadParamErr );
	switch (atom->type) {
		case MP4MovieFragmentHeaderAtomType: self->mfhd = atom; break;
		case MP4TrackFragmentAtomType: err = MP4AddListEntry( atom, self->atomList ); break;
