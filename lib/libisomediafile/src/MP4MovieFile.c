...
static MP4Err LocalPutMovieIntoHandle( MP4Movie theMovie, MP4Handle movieH, u32 include_mdat )
{
	...
	if (movie->prepend_handle) 
		MP4GetHandleSize( movie->prepend_handle, &prepend_size );
		else prepend_size = 0;
	...
	headatom = movie->sgnt;
	if (headatom) {
		err = headatom->calculateSize( headatom ); if (err) goto bail;
	...
	if (mdat) {
		...
		if ((include_mdat) && (mdat->size == 1)) { BAILWITHERROR( MP4NotImplementedErr ); }
		...
		if (moov) {
			err = moov->mdatMoved( moov, 0, mdat->dataSize, move_up ); if (err) goto bail;
		}
		if (meta) {
	...
	headatom = movie->sgnt;
	if (headatom) {
		err = headatom->serialize( headatom, write_location ); if (err) goto bail;
	...
	if (mdat) {
		...
		if (moov) {
			err = moov->mdatMoved( moov, move_up, mdat->dataSize + move_up, - ((s32) move_up) ); if (err) goto bail;
		}
		if (meta) {
			ISOMetaAtomPtr myMeta;
			myMeta = (ISOMetaAtomPtr) meta;
...
MP4WriteMovieToFile( MP4Movie theMovie, const char *filename )
{
	...
	err = LocalPutMovieIntoHandle( theMovie, movieH, 0 ); if (err) goto bail;
...
ISOStartMovieFragment( MP4Movie theMovie )
{
	...
	if ((movie->moovAtomPtr)->type == MP4MovieAtomType) { 
		...
		err = mvex->setSampleDescriptionIndexes( mvex, (MP4AtomPtr) moov ); if (err) goto bail;
	}
	
	err = LocalPutMovieIntoHandle( theMovie, NULL, 1 ); if (err) goto bail;
	...
	err = MP4CreateMovieFragmentAtom( (MP4MovieFragmentAtomPtr *) &moof ); if (err) goto bail;
...
*/
	...
	err = MP4CreateMovieFragmentHeaderAtom( &mfhd ); if (err) goto bail;
