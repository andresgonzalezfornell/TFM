...
static void calculateDefaults (struct MP4TrackRunAtom *self, MP4TrackFragmentHeaderAtomPtr tfhd, u32 flags_index )
{
	MP4Err err;
	...
	if (tfhd->default_sample_duration == 0) 
		tfhd->default_sample_duration = entry->sample_duration;
	if (tfhd->default_sample_size == 0) 
		tfhd->default_sample_size = entry->sample_size;
...
static void setFlags (struct MP4TrackRunAtom *self, MP4TrackFragmentHeaderAtomPtr tfhd )
{
	...
	if (entry->sample_flags != tfhd->default_sample_flags)
	...
	for (i=0, entry=self->entries; i<self->samplecount; i++, entry++ ) {
		...
		if (entry->sample_size != tfhd->default_sample_size) 
		...
		if (entry->sample_composition_time_offset != 0) 
			flags |= trun_sample_composition_times_present;
...
static MP4Err serialize( struct MP4Atom* s, char* buffer )
{
    ...
    if (flags & trun_data_offset_present)				{ PUT32( data_offset ); };
    if (flags & trun_first_sample_flags_present) 		{ PUT32( first_sample_flags ); };
    for (i=0, entry = self->entries; i<self->samplecount; i++, entry++) {
	    if (flags & trun_sample_duration_present) 			{ PUT32_V( entry->sample_duration ); };	    
	    if (flags & trun_sample_size_present) 				{ PUT32_V( entry->sample_size ); };
	    if (flags & trun_sample_flags_present) 				{ PUT32_V( entry->sample_flags ); };
	    if (flags & trun_sample_composition_times_present) 	{ PUT32_V( entry->sample_composition_time_offset ); };
    }
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
    ...
    if (flags & trun_data_offset_present)				{ GET32( data_offset ); };
    if (flags & trun_first_sample_flags_present) 		{ GET32( first_sample_flags ); };
    for (i=0, entry = self->entries; i<self->samplecount; i++, entry++) {
	    if (flags & trun_sample_duration_present) 			{ GET32_V_MSG(( entry->sample_duration ), NULL); };    
	    if (flags & trun_sample_size_present) 				{ GET32_V_MSG(( entry->sample_size ), NULL); };
	    if (flags & trun_sample_flags_present) 				{ GET32_V_MSG(( entry->sample_flags ), NULL); };
	    if (flags & trun_sample_composition_times_present) 	{ GET32_V_MSG(( entry->sample_composition_time_offset ), NULL); };
    }
