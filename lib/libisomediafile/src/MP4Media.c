...
MP4_EXTERN ( MP4Err ) MP4AddMediaSampleReferencePad( MP4Media media, u64 dataOffset, u32 sampleCount,
...
MP4_EXTERN ( MP4Err ) MP4AddMediaSamplesPad( MP4Media media, MP4Handle sampleH, u32 sampleCount,
                                          MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH,
                                          MP4Handle decodingOffsetsH, MP4Handle syncSamplesH,
                                          MP4Handle padsH  )
{
...
MP4_EXTERN ( MP4Err ) MP4GetMediaSample( MP4Media theMedia, MP4Handle outSample, u32 *outSize,
                                         ...
                                         u32 *outSampleFlags )
{
	return MP4GetMediaSampleWithPad( theMedia, outSample, outSize,
                                         desiredDecodingTime, outDecodingTime, outCompositionTime, outDuration,
                                         outSampleDescription, outSampleDescriptionIndex,
                                         outSampleFlags, NULL );
}

MP4_EXTERN ( MP4Err ) MP4GetMediaSampleWithPad( MP4Media theMedia, MP4Handle outSample, u32 *outSize,
                                         u64 desiredDecodingTime, u64 *outDecodingTime, u64 *outCompositionTime, u64 *outDuration,
                                         MP4Handle outSampleDescription, u32 *outSampleDescriptionIndex,
                                         u32 *outSampleFlags, u8* outPad )
{
   ...
   if ( outCompositionTime )
   {
	    ...
	    if ( ctts ) {
	    	err = ctts->getOffsetForSampleNumber( (MP4AtomPtr) ctts, sampleNumber, &compositionTimeOffset ); if (err) goto bail;
	    }
   ...
   *outSize = sampleSize;
   if (outPad) {
     if (padb) 
     { 
	     err = padb->getSamplePad( (MP4AtomPtr) padb, sampleNumber, outPad ); if (err) goto bail; 
	 }
     else *outPad = 0xF8;
   }
   ...
   if ( outDecodingTime )
   {
      *outDecodingTime = sampleDTS; /* was exactSample ? exactSample : nextSample; but exactsample fails to -1 --dws */
   }
   if ( outDuration )
   {
...
{
	return MP4GetIndMediaSampleWithPad( 
		theMedia, sampleNumber, outSample, outSize, outDTS, outCTSOffset, 
		outDuration, outSampleFlags, outSampleDescIndex, NULL);
}
			

MP4_EXTERN ( MP4Err ) MP4GetIndMediaSampleWithPad( MP4Media theMedia,
                                            u32 sampleNumber,
                                            MP4Handle outSample,
                                            u32 *outSize,
                                            u64 *outDTS,
   ...
   )
{
   ...
   if ( outCTSOffset )
   {
      ...
      if ( ctts )
      {
         err = ctts->getOffsetForSampleNumber( (MP4AtomPtr) ctts, sampleNumber, &compositionOffset ); if (err) goto bail;
      }
...
MP4_EXTERN ( MP4Err ) MP4MediaTimeToSampleNum( MP4Media theMedia, u64 mediaTime, u32 *outSampleNum, u64 *outSampleCTS, u64 *outSampleDTS, s32 *outSampleDuration )
{
   ...
   if ( outSampleCTS )	/* was DTS, changed dws 9/3/2001, changed also - to +, if order, and decoding to composition offset */
   {
      s32 compositionOffset;
      compositionOffset = 0;
      if ( ctts )		
...
MP4_EXTERN ( MP4Err ) MP4SampleNumToMediaTime( MP4Media theMedia, u32 sampleNum, u64 *outSampleCTS, u64 *outSampleDTS, s32 *outSampleDuration )
{
   ...
   err = stts->getTimeForSampleNumber( stbl->TimeToSample, sampleNum, &sampleDTS, &sampleDuration ); if (err) goto bail;
   ...
   if ( outSampleCTS ) /* was DTS, decoding offset, ifs in other order, and -;  all changed dws 9/3/2001 */
   {
      ...
      if ( ctts )	
      {
         err = ctts->getOffsetForSampleNumber( (MP4AtomPtr) ctts, sampleNum, &compositionOffset ); if (err) goto bail;
      }
      *outSampleCTS = sampleDTS + compositionOffset;
   }
   if ( outSampleDTS )
