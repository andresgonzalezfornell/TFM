...
static MP4Err testDataEntry( struct MP4MediaInformationAtom *self, u32 dataEntryIndex )
{
	...
	else
	{
		...
		if ( dataEntryAtom == NULL )
			BAILWITHERROR( MP4InvalidMediaErr )	
		err = MP4PreflightDataHandler( self->inputStream, dataEntryAtom ); if (err) goto bail;
	 }
bail:
	TEST_RETURN( err );
...
static MP4Err closeDataHandler( MP4AtomPtr s )
{
	...
	if ( self->dataHandler )
	{
		err = MP4DisposeDataHandler( (MP4DataHandlerPtr) self->dataHandler, dataEntryAtom ); if (err) goto bail;
...
/* Guido : changed because I have now a function to close cleanly */
			err = closeDataHandler((MP4AtomPtr)self); if (err) goto bail;
...
*/
		...
		if ( dataEntryAtom == NULL )
		 ...
		 err = MP4CreateDataHandler( self->inputStream, dataEntryAtom, (struct MP4DataHandler **) &self->dataHandler ); if (err) goto bail;
...
static MP4Err setupNewMedia( struct MP4MediaInformationAtom *self, u32 handlerType, MP4Handle dataH, MP4AtomPtr mdat )
{
	...
	switch ( handlerType )
	{
		case MP4VisualHandlerType:
			err = MP4CreateVideoMediaHeaderAtom( (MP4VideoMediaHeaderAtomPtr*) &mdhd ); if (err) goto bail;
			break;
			
		case MP4AudioHandlerType:
			err = MP4CreateSoundMediaHeaderAtom( (MP4SoundMediaHeaderAtomPtr*) &mdhd ); if (err) goto bail;
			break;
			
		case MP4HintHandlerType:
			err = MP4CreateHintMediaHeaderAtom( (MP4HintMediaHeaderAtomPtr*) &mdhd ); if (err) goto bail;
			break;

		/* Note that MP4TextHandlerType uses the MP4MPEGMediaHeaderAtom, i.e. the default is right */
		default:
			err = MP4CreateMPEGMediaHeaderAtom( (MP4MPEGMediaHeaderAtomPtr*) &mdhd ); if (err) goto bail;
			break;
	}
	if ( mdhd == NULL )
		BAILWITHERROR( MP4InvalidMediaErr )

	err = addAtom( self, mdhd ); if (err) goto bail;
	
	err = MP4CreateDataInformationAtom( &dinf ); if (err) goto bail;
	err = addAtom( self, (MP4AtomPtr) dinf ); if (err) goto bail;
	err = MP4CreateSampleTableAtom( &stbl ); if (err) goto bail;
	err = addAtom( self, (MP4AtomPtr) stbl ); if (err) goto bail;
	err = stbl->setupNew( stbl ); if (err) goto bail;

	/* data information */
	err = MP4CreateDataReferenceAtom( &dref ); if (err) goto bail;
	err = MP4CreateDataEntryURLAtom( &dataEntryAtom ); if (err) goto bail;
	if ( dataH == NULL )
	{
	...
	{
		u32 sz;
		err = MP4GetHandleSize( dataH, &sz ); if (err) goto bail;
	...
	err = dref->addDataEntry( dref, (MP4AtomPtr) dataEntryAtom ); if (err) goto bail;
...
static MP4Err addSampleReference( struct MP4MediaInformationAtom *self, u64 dataOffset, u32 sampleCount,
...
/*		err = MP4ParseAtom( is, &entry ); if (err) goto bail; */
		err = MP4ParseAtomUsingProtoList( is, MP4SampleEntryProtos, MP4GenericSampleEntryAtomType, &entry ); if (err) goto bail;
	...
	err = stbl->addSamples( stbl, sampleCount, dataOffset, durationsH, sizesH, decodingOffsetsH, syncSamplesH, padsH ); if (err) goto bail;
	err = dinf->addSampleReference( dinf, sampleCount, dataReferenceIndex, dataOffset, sizesH ); if (err) goto bail;
bail:
	TEST_RETURN( err );
...
static MP4Err setfieldsize( struct MP4MediaInformationAtom* self, u32 fieldsize )
{
   ...
   err = stbl->setfieldsize( stbl, fieldsize ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
				    ...
				    MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	...
	if (sampleCount > 0) {
		...
		err = stbl->addSamples( stbl, sampleCount, sampleOffset, durationsH, sizesH, decodingOffsetsH, syncSamplesH, padsH ); if (err) goto bail;
		err = dinf->addSamples( dinf, sampleCount, dataReferenceIndex, sampleH ); if (err) goto bail;
	}
bail:
	TEST_RETURN( err );
...
static MP4Err addGroupDescription(struct MP4MediaInformationAtom *self, u32 groupType, MP4Handle description, u32 *index )
{
	MP4Err err;
	MP4SampleTableAtomPtr stbl;
	...
	err = stbl->addGroupDescription( stbl, groupType, description, index ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getGroupDescription(struct MP4MediaInformationAtom *self, u32 groupType, u32 index, MP4Handle description )
{
	MP4Err err;
	MP4SampleTableAtomPtr stbl;
	...
	err = stbl->getGroupDescription( stbl, groupType, index, description ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getSampleGroupMap(struct MP4MediaInformationAtom *self, u32 groupType, u32 sample_number, u32* group_index )
{
	MP4Err err;
	MP4SampleTableAtomPtr stbl;
	...
	err = stbl->getSampleGroupMap( stbl, groupType, sample_number, group_index ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err mdatMoved( struct MP4MediaInformationAtom *self, u64 mdatBase, u64 mdatEnd, s32 mdatOffset )
{
	...
	if ( (stsc == NULL) || (stco == NULL) || (stsd == NULL) )
		BAILWITHERROR( MP4InvalidMediaErr )
