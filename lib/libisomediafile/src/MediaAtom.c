...
MP4Err storeIPMPDescriptorPointers( MP4DescriptorPtr desc,
...
static MP4Err mdatMoved( struct MP4MediaAtom* self, u64 mdatBase, u64 mdatEnd, s32 mdatOffset )
{
   ...
   if ( minf == NULL )
      BAILWITHERROR( MP4InvalidMediaErr );
   err = minf->mdatMoved( minf, mdatBase, mdatEnd, mdatOffset ); if (err) goto bail;	
  bail:
   TEST_RETURN( err );
   return err;
}
				  ...
				  MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
   ...
   err = minf->addSampleReference( minf, dataOffset, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err setfieldsize( struct MP4MediaAtom* self, u32 fieldsize )
{
   ...
   err = minf->setfieldsize( minf, fieldsize ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
			   ...
			   MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
   ...
   */
   ...
   err = minf->addSamples( minf, sampleH, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getGroupDescription(struct MP4MediaAtom *self, u32 groupType, u32 index, MP4Handle description )
{
   MP4Err err;
   MP4MediaInformationAtomPtr minf;
   ...
   err = minf->getGroupDescription( minf, groupType, index, description ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err mapSamplestoGroup(struct MP4MediaAtom *self, u32 groupType, u32 group_index, s32 sample_index, u32 count )
{
   MP4Err err;
   MP4MediaInformationAtomPtr minf;
   ...
   err = minf->mapSamplestoGroup( minf, groupType, group_index, sample_index, count ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getSampleGroupMap(struct MP4MediaAtom *self, u32 groupType, u32 sample_number, u32* group_index )
{
   MP4Err err;
   MP4MediaInformationAtomPtr minf;
   ...
   err = minf->getSampleGroupMap( minf, groupType, sample_number, group_index ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
/*		if (!desc) goto bail;*/
		...
		case MP4ObjectDescriptorUpdateTag:
			...
			for ( i = 0; i < odCount; i++ )	{
                ...
                err = storeIPMPDescriptorPointers( (MP4DescriptorPtr) od,
				...
				for( j = 0; j < esCount; j++ ) {
                    ...
                    /* JB_RESO rewrite IPMPDescriptorPointers */
                    err = storeIPMPDescriptorPointers( (MP4DescriptorPtr) esd, 
                                                        mpod ); if (err) goto bail;
                                    ...
                                    MP4TrackReferenceTypeAtomPtr mpod )
{
    ...
    for ( k = 0; k < ipmpDescPointersCount; k++ )
    {
             ...
             && (ipmpDescPtr->ipmpEsId != 0xFFFF) )
        {
            /* TO DO in libisomp4 : Check if this ES is in the movie
			   ES_ID == TrackID */
            err = mpod->addTrackID(mpod, ipmpDescPtr->ipmpEsId); if (err) goto bail;
            /* change the id */
