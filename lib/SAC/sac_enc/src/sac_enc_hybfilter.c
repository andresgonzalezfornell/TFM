...
void SacEncInitAnaHybFilterbank(tHybFilterStateEnc *hybStateEnc)
{
    int k, n;
    ...
    for (k=0; k<MAX_QMF_BANDS_TO_HYBRID; k++)
    {
        ...
        {
            hybStateEnc->bufferLFReal[k][n] = 0.0;
            hybStateEnc->bufferLFImag[k][n] = 0.0;
        }
    }
    ...
    for (k=0; k<NUM_QMF_BANDS; k++)
    {
        ...
        {
            hybStateEnc->bufferHFReal[k][n] = 0.0;
            hybStateEnc->bufferHFImag[k][n] = 0.0;
        }
    }
}
...
void SacEncApplyAnaHybFilterbank(tHybFilterStateEnc *hybStateEnc,
                              ...
                              float mHybridImag[MAX_TIME_SLOTS][MAX_HYBRID_BANDS])
{
    ...
    for(k=0;k<nrQmfBandsLF;k++)
    {
        ...
        {
            hybStateEnc->bufferLFReal[k][n] = hybStateEnc->bufferLFReal[k][n+nrSamples];
            hybStateEnc->bufferLFImag[k][n] = hybStateEnc->bufferLFImag[k][n+nrSamples];
        }
    }
    ...
    for(k=0;k<nrQmfBandsLF;k++)
    {
        ...
        {
            hybStateEnc->bufferLFReal[k][n+nrSamplesShiftLF] = mQmfReal[n][k];
            hybStateEnc->bufferLFImag[k][n+nrSamplesShiftLF] = mQmfImag[n][k];
        }
    }
    ...
    for(k=0;k<NUM_QMF_BANDS-nrQmfBandsLF;k++)
    {
        ...
        {
            hybStateEnc->bufferHFReal[k][n] = hybStateEnc->bufferHFReal[k][n+nrSamples];
            hybStateEnc->bufferHFImag[k][n] = hybStateEnc->bufferHFImag[k][n+nrSamples];
        }
    }
    ...
    for(k=0;k<NUM_QMF_BANDS-nrQmfBandsLF;k++)
    {
        for(n=0;n<nrSamples;n++)
        {
            hybStateEnc->bufferHFReal[k][n+nrSamplesShiftHF] = mQmfReal[n][k+nrQmfBandsLF];
            hybStateEnc->bufferHFImag[k][n+nrSamplesShiftHF] = mQmfImag[n][k+nrQmfBandsLF];
        }
    }


    sacKChannelFilteringAsym(&(hybStateEnc->bufferLFReal[0][nrSamplesShiftLF+1-PROTO_LEN]),
                             &(hybStateEnc->bufferLFImag[0][nrSamplesShiftLF+1-PROTO_LEN]),
                             mTempOutputReal,
                             mTempOutputImag,
                             nrSamples,
                             8,
    ...
    sacKChannelFilteringAsym(&(hybStateEnc->bufferLFReal[1][nrSamplesShiftLF+1-PROTO_LEN]),
                             &(hybStateEnc->bufferLFImag[1][nrSamplesShiftLF+1-PROTO_LEN]),
                             mTempOutputReal,
                             mTempOutputImag,
                             nrSamples,
                             2,
    ...
    sacKChannelFilteringAsym(&(hybStateEnc->bufferLFReal[2][nrSamplesShiftLF+1-PROTO_LEN]),
                             &(hybStateEnc->bufferLFImag[2][nrSamplesShiftLF+1-PROTO_LEN]),
                             mTempOutputReal,
                             mTempOutputImag,
                             nrSamples,
                             2,
    ...
    for (k=0; k<61; k++)
    {
        for (n=0; n<nrSamples; n++)
        {
            mHybridReal[n][k+10] = hybStateEnc->bufferHFReal[k][n+nrSamplesShiftHF-(PROTO_LEN-1)/2];
            mHybridImag[n][k+10] = hybStateEnc->bufferHFImag[k][n+nrSamplesShiftHF-(PROTO_LEN-1)/2];
        }
    }
}
